...
updated_at = "2026-04-28T00:29:05.120440Z"
epic = "5ea30227"
...
- [x] `apm refresh-epic <id>` exits with a non-zero code and prints an error if the id prefix is ambiguous (matches multiple epic branches)
- [x] `apm refresh-epic <id>` exits with a non-zero code and lists every blocking ticket when any epic ticket is in a state that is not terminal and not `worker_end`
...
- [x] The refresh PR body lists the commits on the default branch that are not yet in the epic branch (one per line, `--oneline` format)
- [x] If an open PR from the default branch into the epic branch already exists, `apm refresh-epic` reports the existing PR number and exits 0 without creating a duplicate
- [x] `apm_core::epic::epic_is_quiescent` is a public function that accepts the repo root, the epic id, the loaded config, and a list of ticket worktrees, and returns the blocking descriptions so `apm epic close` (ticket 056b1ee1) can reuse it without re-implementing the logic
