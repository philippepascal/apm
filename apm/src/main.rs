...
Valid field names:
  ...
  agent       — agent wrapper to use for this ticket; must match an agent
                ...
                state's worker_profile in workflow.toml). Use \"-\" to clear.
...
the given substring:
  apm spec 42 --section \"Acceptance criteria\" --mark \"output is JSON\"")]
    ...
    /// Print the full worker context (system prompt + user message) for a ticket
    #[command(long_about = "Print the complete worker context that would be used if the ticket's\n\
current command:start transition fired, then exit 0. The ticket is not\n\
modified and no worker is spawned.\n\
\n\
By default, both parts are printed with section headers:\n\
\n\
  === system ===\n\
  <system prompt>\n\
\n\
  ...
  <user message>\n\
\n\
Use --system or --message to narrow to one part (no headers).\n\
\n\
SYSTEM PROMPT — three layers, concatenated in order:\n\
\n\
  Layer 1 — Role instructions (cascade, highest priority wins)\n\
...
\n\
  Layer 3 — APM system knowledge\n\
    Generated by `apm instructions --role <role>` (dynamic, role-scoped).\n\
    Covers: state machine, ticket format, session identity, and command reference.\n\
\n\
USER MESSAGE — dependency bundle + ticket:\n\
\n\
  If the ticket has deps, a dependency context bundle is prepended:\n\
  approach sections, commit history, and transitive dep summaries.\n\
  Followed by the agent/role prefix and the serialised ticket content.\n\
\n\
Without a ticket ID, --agent and --role determine the mode:\n\
  • Both provided: print the system prompt for that agent+role. All three\n\
    layers are included when both --agent and --role are provided.\n\
...
\n\
  If a fallback file was used, a note appears inline under layer 1:\n\
    1  .apm/agents/claude/apm.worker.md\n\
       (fallback — .apm/agents/myagent/apm.worker.md not found)\n\
\n\
...
Examples:\n\
  ...
  apm prompt ba121f45 --explain                       # show layer provenance")]
    Prompt {
        ...
        #[arg(long)]
        role: Option<String>,
        /// Print system prompt only (no user message, no section headers)
        #[arg(long, conflicts_with = "message")]
        system: bool,
        /// Print user message only (no system prompt, no section headers)
