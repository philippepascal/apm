...
updated_at = "2026-04-12T17:11:34.993080Z"
epic = "1b029f52"
...
- [x] `apm_core::epic::set_epic_owner(root, epic_id, new_owner, config)` exists as a public function and returns `(usize, usize)` (changed, skipped counts)
- [x] `set_epic_owner` loads all tickets, filters to those belonging to the given epic, skips terminal-state tickets, and bulk-updates the `owner` field by committing to each ticket's branch
- [x] `run_set()` in `epic.rs` delegates the owner-cascade work entirely to `set_epic_owner()`; all ownership-iteration code is removed from the CLI layer
- [x] `apm_core::github::gh_pr_create_or_update` accepts a `body: &str` parameter; the existing caller in `state.rs` passes `&format!("Closes #{id}")` explicitly
- [x] `run_close()` calls `apm_core::github::gh_pr_create_or_update()` with the epic-appropriate body (`"Epic: {epic_branch}"`) and removes its inline `gh pr list` idempotency check and inline `gh pr create` block
- [x] `run_close()` calls `apm_core::epic::epic_id_from_branch()` instead of the inline trim/split expression (dep `aeacd066` must be merged first)
- [x] The local `branch_to_title()` definition is absent from `epic.rs` (removed by dep `aeacd066`); `run_close()` calls `apm_core::epic::branch_to_title()`
- [x] `set_epic_owner` has unit tests covering: happy path (owner updated on non-terminal tickets), skipping terminal tickets
...
| 2026-04-12T10:59Z | in_progress | implemented | claude-0412-1051-e6b0 |
