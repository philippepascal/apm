...
updated_at = "2026-05-30T00:55:31.819601Z"
+++
...
- [x] Tighten the wording on the AC that describes the merge-fails case. Current AC reads: 'apm state ID implemented from merge_failed when the merge fails: ticket stays merge_failed, a history row is appended for the failed attempt, and no implemented state commit is written.' The bolded last clause (no implemented state commit is written) is literally false. Trace state.rs::transition exactly: line 166 writes the merge_failed -> implemented commit on the ticket branch BEFORE the match completion block runs. When PrOrEpicMerge calls merge_into_default and it fails, the on_failure path (lines 200-216 for PrOrEpicMerge) writes a SECOND commit on top: implemented -> merge_failed. So the ticket branch ends up with TWO commits: the implemented row followed by the merge_failed row. A test written to the literal current AC would fail because the implemented state commit IS written.

Rewrite the AC to describe what actually happens. Suggested wording: 'apm state ID implemented from merge_failed when the merge fails: the ticket branch ends with two new commits — first merge_failed -> implemented (line 166), then implemented -> merge_failed via the on_failure path; the ticket frontmatter final state is merge_failed; the History table contains both the failed implemented transition row and the on_failure merge_failed row; no work commits land on target_branch.' The implementer's test should assert the FINAL state on the ticket branch and the presence of both history rows, not the absence of an implemented commit.

The other AC (merge succeeds) is correct as written and needs no change.
...
| 2026-05-30T00:52Z | specd | ammend | philippepascal |
| 2026-05-30T00:53Z | ammend | in_design | philippepascal |
