...
updated_at = "2026-04-12T10:24:32.488798Z"
epic = "1b029f52"
...
- [x] `apm init` still adds all `APM_ALLOW_ENTRIES` to `.claude/settings.json` when the user answers `y`
- [x] `apm init` still adds all `APM_USER_ALLOW_ENTRIES` to `~/.claude/settings.json` when the user answers `y`
- [x] `apm init` still skips patching `.claude/settings.json` when the file does not exist
- [x] `apm init` still creates `~/.claude/settings.json` (including parent dir) when the file does not exist and the user answers `y`
- [x] `apm init` still skips patching when all target entries are already present (no prompt shown)
- [x] `apm init` still skips patching when the user answers `n` or presses enter at the `[y/N]` prompt
- [x] `init.rs` contains no duplicate `permissions/allow` manipulation logic — there is exactly one function that reads, diffs, prompts, and writes a settings file
- [x] The `update_settings_json` helper is `fn update_settings_json(path: &Path, entries: &[&str], prompt_header: &str, prompt_confirm: &str, updated_msg: &str, create_if_missing: bool) -> Result<()>`
...
| 2026-04-12T09:14Z | in_design | specd | claude-0412-0912-e6a8 |
