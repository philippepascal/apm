...
updated_at = "2026-04-09T00:50:18.800223Z"
+++
...
- [x] `cargo clippy --package apm-core -- -D warnings` exits with code 0
- [x] `archive.rs` `double_ended_iterator_last` warning is gone (`.last()` replaced with `.next_back()`)
- [x] `start.rs` `too_many_arguments` warning on `spawn_container_worker` is suppressed with `#[allow(clippy::too_many_arguments)]`
- [x] `ticket.rs` `too_many_arguments` warning on `pick_next` is suppressed with `#[allow(clippy::too_many_arguments)]`
- [x] `ticket.rs` `too_many_arguments` warning on `create` is suppressed with `#[allow(clippy::too_many_arguments)]`
- [x] `ticket.rs` `unnecessary_map_or` warnings are gone — all instances of `.map_or(true, |x| …)` replaced with `.is_none_or(|x| …)` and `.map_or(false, |x| …)` replaced with `.is_some_and(|x| …)`
- [x] `ticket.rs` `manual_strip` warnings are gone — `starts_with(prefix)` + index slice replaced with `strip_prefix(prefix)`
- [x] All existing apm-core tests continue to pass (`cargo test --package apm-core`)
