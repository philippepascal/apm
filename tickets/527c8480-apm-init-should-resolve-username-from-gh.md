...
updated_at = "2026-04-24T08:01:35.345878Z"
+++
...
- [x] When `gh` is authenticated, `apm init` prompts `Username [<gh-login>]:` where `<gh-login>` is the value returned by `gh api user -q .login`
- [x] Pressing Enter at the prompt when a gh-supplied default is shown accepts that default and writes it to `.apm/local.toml`
- [x] Typing a value at the prompt overrides the gh default; the typed value is written to `.apm/local.toml`
- [x] When `gh` is not installed, the prompt falls back to `Username []:` and blank-default behaviour is unchanged
- [x] When `gh api user -q .login` exits non-zero (unauthenticated or API error), the prompt falls back to `Username []:` and blank-default behaviour is unchanged
- [x] When `gh api user -q .login` exits zero but returns an empty string, the prompt falls back to `Username []:`
- [x] All existing conditions that skip the prompt entirely — `has_git_host` true, not a TTY, `.apm/local.toml` already exists — are unaffected by this change
