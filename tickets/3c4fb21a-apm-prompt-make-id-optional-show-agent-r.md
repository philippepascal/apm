...
updated_at = "2026-05-22T10:18:38.559251Z"
+++
...
- [x] `apm prompt` with no arguments exits 0 and prints an `Agents:` line whose value is the sorted, comma-space-separated list of subdirectory names under `.apm/agents/`
- [x] `apm prompt` with no arguments exits 0 and prints a `Roles:` line whose value is the sorted, comma-space-separated list of unique role names extracted from `apm.<role>.md` filenames across all agent directories
- [x] The two output lines align their values at the same column (labels padded to equal width)
...
- [x] `apm prompt <id> --agent <a> --role <r>` continues to work as before
...
| 2026-05-22T08:14Z | ready | in_progress | philippepascal |
| 2026-05-22T08:20Z | in_progress | implemented | claude-0522-0814-b3d9 |
| 2026-05-22T10:18Z | implemented | closed | philippepascal(apm-sync) |
