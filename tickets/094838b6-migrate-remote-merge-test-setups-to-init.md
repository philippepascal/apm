...
updated_at = "2026-05-03T21:36:33.869766Z"
epic = "0b1c71db"
...
- [x] `setup_squash_remote()` body contains no `std::fs::write` call that writes `apm.toml`
- [x] `setup_pr_or_epic_merge_remote()` body contains no `std::fs::write` call that writes `apm.toml`
- [x] `setup_merge_strategy_remote()` body contains no `std::fs::write` call that writes `apm.toml`
- [x] A new private helper `init_remote_repo() -> (TempDir, TempDir)` exists that creates a bare remote + local clone via `apm init --no-claude --quiet`
- [x] `squash_merge_config()`, `pr_or_epic_merge_config_toml()`, and `merge_strategy_config_toml()` are removed (they become dead code after migration)
- [x] The squash-merge detection tests `sync_detect_squash_merged_branch_remote_ref_present` and `sync_detect_squash_merged_branch_remote_ref_deleted` pass
- [x] The pr_or_epic_merge tests `pr_or_epic_merge_with_target_branch_merges_into_target`, `pr_or_epic_merge_without_target_branch_attempts_pr`, and `pr_or_epic_merge_with_target_branch_pushes_target_to_origin` pass
- [x] The merge-strategy test `merge_strategy_merges_locally_without_push` passes
- [x] `setup_merge_strategy_remote()` includes a `// BYPASS:` comment explaining why `.apm/workflow.toml` is edited directly
- [x] The bypass in `setup_merge_strategy_remote()` edits `.apm/workflow.toml` (not `apm.toml`)
...
| 2026-05-03T21:31Z | ready | in_progress | philippepascal |
| 2026-05-03T21:36Z | in_progress | implemented | claude-0503-2131-1b20 |
