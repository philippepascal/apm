...
- [x] `.github/workflows/ci.yml` contains a job named `sast` that is triggered on both `push` and `pull_request` events
- [x] The `sast` job declares `permissions: security-events: write` at the job level so the SARIF upload to GitHub Security is authorised
- [x] The `sast` job initialises CodeQL with `languages: [rust]` and `queries: security-extended` via `github/codeql-action/init@v3`
- [x] The `sast` job builds the workspace with `cargo build --workspace` (build tracing lets CodeQL follow the compiled output)
- [x] The `sast` job calls `github/codeql-action/analyze@v3` with `category: /language:rust`; SARIF results appear in the GitHub Security → Code scanning tab
- [x] The `sast` job exits non-zero if CodeQL cannot complete analysis (tool error, build failure), failing the CI check
- [x] The root `spec.md` `## CI gate` section is updated to document the `sast` job alongside the existing `ci` and `audit` jobs
