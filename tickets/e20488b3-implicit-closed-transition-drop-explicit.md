...
updated_at = "2026-06-01T14:19:05.858310Z"
+++
...
- [x] `apm state <id> closed` succeeds from `new` without a `to = "closed"` entry in workflow.toml
- [x] `apm state <id> closed` succeeds from `groomed` without a `to = "closed"` entry in workflow.toml
- [x] `apm state <id> closed` succeeds from `question` without a `to = "closed"` entry in workflow.toml
- [x] `apm state <id> closed` succeeds from `specd` without a `to = "closed"` entry in workflow.toml
- [x] `apm state <id> closed` succeeds from `ammend` without a `to = "closed"` entry in workflow.toml
- [x] `apm state <id> closed` succeeds from `in_design` without a `to = "closed"` entry in workflow.toml
- [x] `apm state <id> closed` succeeds from `ready` without a `to = "closed"` entry in workflow.toml
- [x] `apm state <id> closed` succeeds from `in_progress` without a `to = "closed"` entry in workflow.toml
- [x] `apm state <id> closed` succeeds from `blocked` without a `to = "closed"` entry in workflow.toml
- [x] `apm state <id> closed` succeeds from `implemented` without a `to = "closed"` entry in workflow.toml
- [x] `apm state <id> closed` succeeds from `merge_failed` without a `to = "closed"` entry in workflow.toml
- [x] `apm state <id> closed` fails with a clear error when the ticket is already in state `closed`
- [x] `apm validate` rejects a workflow.toml that contains an explicit `to = "closed"` (or any terminal state) transition, and the error message names both the source state and the terminal target
- [x] `apm-core/src/default/workflow.toml` contains zero `[[workflow.states.transitions]]` blocks with `to = "closed"`
...
| 2026-06-01T08:09Z | merge_failed | implemented | claude |
| 2026-06-01T14:19Z | implemented | closed | philippepascal(apm-sync) |
