...
**Step 1 — Rename and restructure the core function**\n\nRename `resolve_system_prompt()` to `build_system_prompt()` in `apm-core/src/start.rs`. Change the priority cascade so the per-agent filesystem file wins first:\n\n- Level 0: `.apm/agents/<agent>/apm.<role>.md` — per-agent override (soft: skip if absent)\n- Level 1: transition.instructions — hard error if path set but file missing\n- Level 2: profile.instructions — hard error if path set but file missing\n- Level 3: workers.instructions — hard error if path set but file missing\n- Level 4: built-in default (resolve_builtin_instructions)\n- Level 5: error: no instructions found\n\nAll existing error messages remain the same for levels 1–5. Level 0 is silent on miss (same semantics as the old Level 3 soft check).\n\n**Step 2 — Add the `apm prompt` subcommand**\n\nIn `apm/src/main.rs`, add a `Prompt` variant with fields: `id: String`, `--agent: Option<String>`, `--role: Option<String>`. Wire to a new `prompt::run()` in `apm-core/src/prompt.rs` (or a public function in `start.rs` — implementer's choice). The function: (1) loads the ticket for `id`; (2) finds the applicable transition for the ticket's current state using the same logic as `run()`; (3) resolves agent and role (transition → profile → workers → default) then applies CLI overrides; (4) calls `build_system_prompt()`; (5) writes the result to stdout; (6) returns non-zero if resolution fails.\n\n**Step 3 — Update the three spawn paths**\n\nIn `run()` (line 362), `run_next()` (line 566), and `spawn_next_worker()` (line 770), replace the `resolve_system_prompt(...)` calls with `build_system_prompt(...)`. No other changes to these call sites — the signature change is rename plus the reordered levels internally.\n\n**Step 4 — No config or file changes**\n\nThe `.apm/workflow.toml` and `.apm/config.toml` formats are unchanged. No migration needed. Existing transitions that set `instructions = ...` continue to work as Level 1 when no per-agent file is present.
