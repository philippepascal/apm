...
- [ ] Add d3ebdc0f as a dependency — both tickets independently create `apm/src/util.rs` and add `pub mod util;` to `lib.rs`. Without this dependency, whichever lands second will hit a merge conflict or silently overwrite the first ticket's content.
