...
updated_at = "2026-04-02T22:43:28.741210Z"
+++
...
- [x] Add `satisfies_deps = "spec"` to `ready`, `in_progress`, and `ammend` states in `workflow.toml` — not just `specd`. All three states have a complete spec; without this, a `groomed` ticket A becomes re-blocked the moment its dep B advances past `specd` to `ready` or `in_progress`, which is the opposite of the intended behaviour.
- [x] Remove the duplicate approach content. The full plan is already written as `####` subsections under `### Approach`; the identical content is repeated as top-level `###` sections below it. Delete the duplicates.
...
| 2026-04-02T22:30Z | specd | ammend | apm |
| 2026-04-02T22:30Z | ammend | in_design | apm |
...
| 2026-04-02T22:43Z | implemented | closed | apm |
