...
updated_at = "2026-05-25T06:56:47.976116Z"
+++
...
- [x] `apm start` and `apm start --next` fall back to `claude/coder` (not `claude/worker`) when no `worker_profile` is set on the transition and no `default` is set in `[workers]`
- [x] `apm init` generates `.apm/config.toml` with `default = "claude/coder"` in `[workers]`
- [x] `apm init` generates `.apm/workflow.toml` with `worker_profile = "claude/coder"` on the `ready → in_progress` transition
- [x] `apm init` writes `.apm/agents/claude/apm.coder.md` (not `apm.worker.md`)
- [x] The built-in instruction cascade resolves `claude/coder`, `mock-happy/coder`, `mock-sad/coder`, `mock-random/coder`, and `debug/coder` without error
- [x] The live `.apm/config.toml` contains `default = "claude/coder"`
- [x] The live `.apm/workflow.toml` contains `worker_profile = "claude/coder"` on the `ready → in_progress` transition
- [x] The live `.apm/agents/claude/` directory contains `apm.coder.md` and no `apm.worker.md`
- [x] `cargo test --workspace` passes with all assertions updated to `"claude/coder"`
...
| 2026-05-25T01:41Z | specd | ammend | philippepascal |
...
| 2026-05-25T02:03Z | in_progress | implemented | claude |
| 2026-05-25T06:56Z | implemented | closed | philippepascal(apm-sync) |
