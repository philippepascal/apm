...
updated_at = "2026-04-18T07:39:00.650903Z"
depends_on = ["b15354a6"]
+++
...
- [x] When `sync_non_checked_out_refs` finds one or more ahead ticket/* or epic/* branches, stdin is a TTY, and `--quiet` is not set, `apm sync` prints a single bundled prompt: `push N ahead branch(es) to origin now? [y/N]`
- [x] Answering `y` at the bundled prompt causes `apm sync` to push each ahead branch and print a summary line
...
- [x] POST `/api/sync` with body `{ "push_default": true }` when `<default>` is diverged does not push and returns the diverged warning in `log`
- [x] POST `/api/sync` with body `{ "push_refs": true }` pushes all ahead ticket/* and epic/* branches and appends per-branch confirmation lines to `log`
...
- [x] Clicking `Push <default>` sends POST `/api/sync` with `{ "push_default": true }`, replaces the log display with the new response, and invalidates ticket queries
- [x] When `ahead_branches` contains non-default branches, the Sync modal renders a `Push N ahead branch(es)` button
...
| 2026-04-18T07:11Z | ready | in_progress | philippepascal |
| 2026-04-18T07:20Z | in_progress | implemented | claude-0418-0711-75a0 |
| 2026-04-18T07:39Z | implemented | closed | apm-sync |
