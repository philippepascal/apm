...
updated_at = "2026-04-08T00:37:18.063927Z"
epic = "ac0fb648"
...
- [x] `ticket::close()` no longer contains inline history-appending code; it calls `state::append_history()` instead
- [x] `review::available_transitions()` is moved to `state.rs`; `review.rs` re-exports it so existing callers compile unchanged
- [x] A `provision_worktree` helper exists in `state.rs` (or `git.rs`) and is called by both the `in_design` branch in `state::transition()` and by `start::run()` — neither site contains its own `ensure_worktree` + `sync_agent_dirs` pair
- [x] `cargo test` passes with no regressions after all changes
- [x] A contributor reading only `state.rs` can find: history appending, amendment section insertion, available-transition filtering, worktree provisioning, completion strategy execution, and all state-entry validations — without needing to open `ticket.rs` or `review.rs`
...
| 2026-04-08T00:33Z | ready | in_progress | philippepascal |
| 2026-04-08T00:37Z | in_progress | implemented | claude-0408-0033-f5f8 |
