...
updated_at = "2026-05-04T20:13:03.687186Z"
+++
...
- [x] `GET /api/workflow` returns a JSON object with a `states` array (each entry: `id`, `label`, `terminal`, `actionable`) and a `transitions` array (each entry: `from`, `to`, `label`, `trigger`) reflecting the project's live `WorkflowConfig`
- [x] `GET /api/workflow` returns `{"states":[],"transitions":[]}` without error when the server is running in `InMemory` mode (no git root / no config file)
- [x] A "Workflow" button appears in the `SupervisorView` header alongside the existing Sync and Clean buttons
- [x] Clicking "Workflow" opens a modal that displays the workflow graph
- [x] The graph renders every state returned by `/api/workflow` as a labelled node
- [x] Terminal states are visually distinguished from non-terminal states (e.g. different border or opacity)
- [x] Node fill or border colour matches the colour already used for that state in `stateColors.ts` (the `dot` palette entry)
- [x] The graph renders every transition as a directed arrow from source node to target node
- [x] Each transition arrow carries a label (the transition's `label` field, falling back to `"→ <to>"` when blank)
- [x] Nodes are positioned using a layer-based layout computed at render time from the graph topology; no x/y coordinates are hard-coded in the component
- [x] The graph is rendered as plain SVG — no new npm graph-layout or rendering library is added to `package.json`
- [x] When `/api/workflow` returns an empty `states` array, the modal shows a "No workflow configured" message instead of a blank SVG
...
| 2026-05-04T17:45Z | in_design | specd | claude-0504-1741-3358 |
| 2026-05-04T17:48Z | specd | ammend | philippepascal |
| 2026-05-04T17:48Z | ammend | in_design | philippepascal |
...
| 2026-05-04T20:13Z | implemented | closed | philippepascal(apm-sync) |
