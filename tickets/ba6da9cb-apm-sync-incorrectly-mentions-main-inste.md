...
updated_at = "2026-06-13T18:10:07.590423Z"
+++
...
- [x] When a ticket has no `target_branch` and its branch is unmerged, the hint names the project's configured default branch (e.g. `develop`, `trunk`) rather than the literal string `main`.
- [x] When a ticket has a `target_branch` set (e.g. an epic branch like `epic/abc-user-auth`) and its branch is unmerged, the hint names that `target_branch` instead of `main`.
- [x] The hint message text is otherwise unchanged: it still includes the ticket id and the `apm state <id> closed` recovery command.
- [x] All existing `cargo test --workspace` tests continue to pass.
...
| 2026-06-12T07:59Z | in_design | specd | claude |
| 2026-06-12T22:52Z | specd | ready | philippepascal |
| 2026-06-12T22:53Z | ready | in_progress | philippepascal |
| 2026-06-12T23:00Z | in_progress | implemented | claude |
| 2026-06-13T18:10Z | implemented | closed | philippepascal(apm-sync) |
