...
updated_at = "2026-05-23T00:30:30.927661Z"
epic = "ab6e5db7"
...
Two additional gaps: (1) T4 (34ad9126) deletes the built-in apm-core/src/default/agents/claude/apm.spec-writer.md and updates resolve_builtin_instructions — init.rs:149 writes that file from the built-in via include_str!; after T4 that include_str! will not compile. This ticket must also stop writing .apm/agents/claude/apm.spec-writer.md from setup(). (2) ensure_claude_md (init.rs:363) currently adds a single @-import line; after the redesign CLAUDE.md needs two separate @-imports (apm.project.md and apm.main-agent.md). The function signature must accept multiple paths or be called twice, and the inject logic must handle the case where one import already exists but the other does not (idempotent partial state).
