...
updated_at = "2026-04-18T07:38:38.789786Z"
+++
...
- [x] `apm state <id> groomed` run by a user with `USER=alice` and no `APM_AGENT_NAME` set records `alice` in the history `By` column
- [x] `apm review` approval that triggers `specd → ready` records the invoking user's identity (not `"apm"`) in the history `By` column
- [x] `apm sync` auto-closing an `implemented` ticket records `<user>(apm-sync)` (e.g. `philippepascal(apm-sync)`) in the history `By` column
- [x] `apm sync` run by a Claude agent with `APM_AGENT_NAME=claude-xyz` records `claude-xyz(apm-sync)` in the history `By` column
- [x] `apm close <id>` run by a user with `USER=alice` and no `APM_AGENT_NAME` set records `alice` in the history `By` column
- [x] When `APM_AGENT_NAME` is set, `apm state` still records that agent name as the actor (existing agent behaviour is preserved)
- [x] Server-side sync handler behaviour is unchanged
