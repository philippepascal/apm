...
updated_at = "2026-04-02T01:37:24.363161Z"
+++
...
- [ ] `derive_epic_state` in the Approach hardcodes "in_design", "in_progress", "accepted", "closed", "implemented". Replace with config-driven logic: the server loads workflow config and passes `&config.workflow.states` to the helper; the helper uses `actionable`, `satisfies_deps`, and `terminal` flags (same rules as a5e1ea24). No state ID string comparisons.
