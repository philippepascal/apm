...
updated_at = "2026-06-01T00:20:32.458560Z"
epic = "9c3c4c20"
...
- [x] A workflow.toml with `worker_profile` under any `[[workflow.states.transitions]]` block fails to parse
- [x] The parse error message names `worker_profile` and tells the user to move it to the state block
- [x] A workflow.toml with `worker_profile` only at the state level parses correctly
- [x] `apm start` resolves the worker profile via `state.worker_profile` → `workers.default` → built-in; no transition-level lookup occurs
- [x] `configured_agent_names` collects agent names from state-level `worker_profile` only
- [x] `format_live_state_machine` filters transitions by role using only state-level `worker_profile`; `derive_transition_role` is deleted
- [x] `apm-core/src/default/workflow.toml` contains no `worker_profile` keys under any transition block
- [x] `.apm/workflow.toml` contains no `worker_profile` keys under any transition block
