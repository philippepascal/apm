...
updated_at = "2026-04-12T07:57:58.163187Z"
epic = "57bce963"
...
- [x] `apm-core/src/worktree.rs` exists and is declared as `pub mod worktree` in `apm-core/src/lib.rs`
- [x] `find_worktree_for_branch` is defined in `worktree.rs` and absent from `git_util.rs`
- [x] `list_ticket_worktrees` is defined in `worktree.rs` and absent from `git_util.rs`
- [x] `ensure_worktree` is defined in `worktree.rs` and absent from `git_util.rs`
- [x] `add_worktree` is defined in `worktree.rs` and absent from `git_util.rs`
- [x] `remove_worktree` is defined in `worktree.rs` and absent from `git_util.rs`
- [x] `sync_agent_dirs` is defined in `worktree.rs` and absent from `git_util.rs`
- [x] Private helpers `clean_agent_dirs`, `is_tracked`, and `copy_dir_recursive` are defined in `worktree.rs` and absent from `git_util.rs`
- [x] `provision_worktree` is defined as `pub fn` in `worktree.rs` and absent from `state.rs`
- [x] `list_worktrees_with_tickets` is defined as `pub fn` in `worktree.rs` and absent from `ticket.rs`
- [x] Every call-site that previously referenced `git::find_worktree_for_branch`, `git::list_ticket_worktrees`, `git::ensure_worktree`, `git::add_worktree`, `git::remove_worktree`, or `git::sync_agent_dirs` is updated to `worktree::`
- [x] Every call-site that previously referenced `git::provision_worktree` or `state::provision_worktree` is updated to `worktree::provision_worktree`
- [x] Every call-site that previously referenced `ticket::list_worktrees_with_tickets` or `git::list_worktrees_with_tickets` is updated to `worktree::list_worktrees_with_tickets`
...
| 2026-04-12T07:06Z | in_design | specd | claude-0412-0704-b558 |
| 2026-04-12T07:13Z | specd | ready | apm |
| 2026-04-12T07:49Z | ready | in_progress | philippepascal |
| 2026-04-12T07:57Z | in_progress | implemented | claude-0412-0749-c630 |
