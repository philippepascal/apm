...
updated_at = "2026-06-02T18:03:16.366178Z"
+++
...
- [x] `apm epic submit <id>` on an epic with no existing PR pushes the branch to origin and opens a PR; output includes the PR URL.
- [x] `apm epic submit <id>` on an epic with an existing open PR updates the PR without creating a new one; output includes the existing PR URL.
- [x] `apm epic submit --merge <id>` on an epic that merges cleanly into the default branch merges it locally and pushes; no PR is created.
- [x] `apm epic submit --merge <id>` on an epic that would conflict exits non-zero with a message naming the conflict and suggesting `--pr`.
- [x] `apm epic submit --auto <id>` merges cleanly when no conflict would occur; falls back to opening a PR when a conflict is detected.
- [x] `apm epic close <id>` on an epic whose branch is fully merged into `origin/<default>` (regular or squash merge) deletes the local branch, removes the worktree if present, and exits zero.
- [x] `apm epic close <id>` on an epic with commits not yet in `origin/<default>` exits non-zero, prints the number of unmerged commits, and suggests `apm epic close --force`.
- [x] `apm epic close <id>` on an epic where any ticket has an active worker (worktree present and PID alive) exits non-zero, names the affected ticket(s) and PID(s), and suggests `apm epic close --force`.
- [x] `apm epic close --force <id>` deletes the local branch and removes the worktree unconditionally, bypassing both the live-worker check and the unmerged-commits check.
- [x] `apm sync` output includes an "Epics ready to submit" section listing epics whose derived state is `done` and whose branch is not yet merged into `origin/<default>`.
- [x] `apm sync` output includes an "Epics ready to close" section listing epics whose branch is merged into `origin/<default>` (squash-aware detection).
- [x] `apm sync` prints the epic hint sections without prompting; no action is taken automatically.
- [x] `apm clean --epics` exits non-zero with a message directing the user to `apm epic close <id>`.
- [x] A public function `apm_core::git::is_branch_content_merged(root, default_branch, branch)` exists, prefers `origin/<default>` when the remote ref is present, and falls back to the local ref when it is not.
- [x] `is_branch_content_merged` returns `true` for a branch merged into default via a regular (fast-forward or no-ff) merge.
- [x] `is_branch_content_merged` returns `true` for a branch squash-merged into default.
- [x] `is_branch_content_merged` returns `false` for a branch with commits not present in default.
- [x] Integration test covers the end-to-end flow: `apm epic submit --merge` merges the branch; `apm epic close` subsequently deletes the branch and worktree.
- [x] Integration test verifies that `sync::detect` populates `epic_close_hints` after a squash merge of an epic branch, and `epic_submit_hints` when the epic is done but not yet merged.
- [x] `apm epic --help` shows `submit` and `close` as distinct subcommands with non-overlapping descriptions.

### Out of scope

- Epic state machine (no `merge_failed`-equivalent state; a conflict in `--merge` is an error, not a state transition)
- The non-terminal-ticket portion of the quiescence check in `run_close` is dropped; `submit` and `close` no longer gate on whether tickets are in terminal states. The live-worker portion is kept (see Approach Step 3).
...
- [x] Keep a live-worker safety check on apm epic close (without --force). If any ticket in the epic has an active worker (worktree present plus alive PID, same check epic_is_quiescent uses today), refuse the close with a clear error naming the affected ticket(s) and worker PID(s). The current run_close has this check via epic_is_quiescent — only the non-terminal-ticket portion of that check should be dropped per the new design; the live-worker portion should be kept. --force bypasses this check as well, matching --force's role as the universal supervisor escape hatch. Add an AC verifying close refuses when a live worker exists on an in_progress ticket in the epic; another AC for --force overriding it.
...
| 2026-06-02T17:30Z | specd | ammend | philippepascal |
| 2026-06-02T17:30Z | ammend | in_design | philippepascal |
| 2026-06-02T17:35Z | in_design | specd | claude |
| 2026-06-02T17:41Z | specd | ready | philippepascal |
| 2026-06-02T17:47Z | ready | in_progress | philippepascal |
| 2026-06-02T18:03Z | in_progress | implemented | claude |
