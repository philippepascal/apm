...
updated_at = "2026-04-28T15:14:07.086559Z"
+++
...
- [x] `[agents] max_workers_on_default` is accepted in `.apm/config.toml` without error
- [x] When the field is absent, it defaults to `1`
- [x] When set to `1`, a second non-epic ticket is not picked by `apm start --next` while one non-epic worker is already active
- [x] When set to `2`, a third non-epic ticket is not picked while two non-epic workers are active
- [x] When set to `0`, non-epic tickets are picked freely up to `max_concurrent` (no additional cap)
- [x] Epic-linked tickets are unaffected: `max_workers_per_epic` still governs them independently
- [x] The `apm work` daemon respects the limit in its spawn loop (non-epic slots are counted each iteration)
- [x] A unit test covers: limit=1, 0 active non-epic workers → not blocked
- [x] A unit test covers: limit=1, 1 active non-epic worker → blocked
- [x] A unit test covers: limit=0, any number of active non-epic workers → not blocked
- [x] A unit test covers: active workers are all epic-linked → non-epic slot is not blocked
...
| 2026-04-28T15:11Z | in_design | specd | claude-0428-1508-c3f0 |
