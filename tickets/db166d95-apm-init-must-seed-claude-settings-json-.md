...
- [x] `apm init --yes` on a repo with no `.claude/` directory does not create `.claude/` or `settings.json`, and exits zero.
- [x] `apm init` (no `--yes`) on a non-TTY stdin with `.claude/settings.json` absent still creates and seeds the file (non-interactive path does not prompt).
- [x] `apm init --yes` updates `~/.claude/settings.json` with `APM_USER_ALLOW_ENTRIES` without prompting.
- [x] `apm init --yes` prints `Updated .claude/settings.json` when the project file was created or modified, and prints `Updated ~/.claude/settings.json` when the user file was modified.
- [x] `apm init --no-claude` still suppresses all settings.json writes even when `--yes` is passed.
