...
updated_at = "2026-04-08T23:47:27.364929Z"
epic = "18dab82d"
...
- [x] `apm epic set <epic-id> owner <user>` changes owner on all non-closed tickets in the epic
- [x] Closed (terminal) tickets are skipped
- [x] Output lists each ticket that was changed and any that were skipped
- [x] The ownership check applies: current user must be the owner of each ticket being changed
- [x] If any ticket fails the ownership check, none are changed (atomic: all or nothing)
- [x] Owner validation (collaborator check) applies to the new owner
- [x] Tests cover: bulk change succeeds, closed tickets skipped, non-owner blocked
