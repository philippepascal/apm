...
updated_at = "2026-04-04T18:22:58.036104Z"
+++
...
- [x] \`default_workflow_toml()\` parses without error when deserialized as \`WorkflowFile\` (the internal serde struct used by \`Config::load\`)
- [x] The parsed workflow contains exactly the eleven expected state ids: new, groomed, question, specd, ammend, in_design, ready, in_progress, blocked, implemented, closed
- [x] Every state that should carry \`dep_requires\` (groomed, ammend) has a non-None value after parsing
- [x] Every state that should carry \`satisfies_deps\` (specd, ammend, in_design, ready, in_progress, implemented) has a non-default value after parsing
- [x] \`default_ticket_toml()\` parses without error when deserialized as \`TicketFile\` (the internal serde struct used by \`Config::load\`)
- [x] The parsed ticket config contains the four required sections: Problem, Acceptance criteria, Out of scope, Approach, all with \`required = true\`
- [x] \`ensure_gitignore\` creates a file that contains all five expected entries: tickets/NEXT_ID, .apm/local.toml, .apm/*.init, .apm/sessions.json, .apm/credentials.json
...
| 2026-04-04T16:09Z | in_progress | implemented | claude-0404-1600-impl1 |
| 2026-04-04T18:22Z | implemented | closed | apm-sync |
