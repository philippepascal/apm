...
updated_at = "2026-04-24T07:53:19.046181Z"
+++
...
- [x] `apm state <epic-id> in_design` run from inside any linked worktree places the new epic worktree at `<worktrees_base>/epic-<id>-<slug>/`
- [x] `apm state <epic-id> in_design` run from the main repo root places the new epic worktree at `<worktrees_base>/epic-<id>-<slug>/`
- [x] `merge_into_default` invoked from inside a linked worktree creates the default-branch worktree at `<worktrees_base>/<default-branch>/`, not at `<worktrees_base>/<worktrees_base>/...`
- [x] `apm init` / `ensure_worktrees_dir` run from inside a linked worktree creates `<worktrees_base>/` at the correct location (sibling of the main repo, not nested under the existing worktrees dir)
- [x] Ticket (non-epic) worktrees continue to land at `<worktrees_base>/ticket-<id>-<slug>/` with no regression
- [x] `cargo test` passes with no new failures after both callsite fixes
...
| 2026-04-24T07:50Z | ready | in_progress | philippepascal |
| 2026-04-24T07:53Z | in_progress | implemented | claude-0424-0750-2c98 |
