...
updated_at = "2026-05-29T03:30:08.449615Z"
+++
...
- [x] `Config::implementation_state_ids()` returns the union of (a) `to` targets of `command:start` transitions whose `worker_profile` does not end with `/spec-writer`, and (b) `to` targets of `Pr`/`Merge`/`PrOrEpicMerge` completion transitions; for the default workflow it returns `{in_progress, implemented}`
- [x] `Config::implementation_state_ids()` is invariant to the order in which `[[workflow.states]]` are listed — a unit test builds the default workflow, then builds it again with the state entries shuffled, and asserts an identical result
