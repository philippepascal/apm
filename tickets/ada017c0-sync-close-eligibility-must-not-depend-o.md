...
- [ ] CORRECTION to the previous amendment note (three words were dropped by a shell-quoting error): (1) 'the SET of targets' should read 'the SET of to-targets' — i.e. the set of destination states (the transition 'to' field) of command:start non-spec-writer transitions. (2) 'Do not rely on the field being present' should read 'Do not rely on the outcome field being present' — referring to the transition outcome field (success/needs_input/cancelled). (3) 'a ticket whose fork reached main' should read 'a new-state ticket whose fork reached main' — the side-note case that starts and stays in the new state.
