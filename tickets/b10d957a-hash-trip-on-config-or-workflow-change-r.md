...
updated_at = "2026-04-27T22:25:17.066407Z"
epic = "5ea30227"
...
- [ ] Add `Verify` to `is_read_only_command` so `apm verify` runs (with the warning) when config has changed and validation is failing. The "ok to run when config is broken" set is: `list`, `show`, `next`, `verify`, plus `validate` (already exempt). Everything else is blocked.
