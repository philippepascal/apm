...
updated_at = "2026-04-28T20:17:20.514245Z"
epic = "e3b24cb9"
...
- [ ] Spec pre-commits to schemars 0.8 in the Approach without framing it as a deliberate choice. Reframe: add a "Decision" subsection at the top of Approach stating "schemars 0.8 is chosen because (1) it derives from `JsonSchema` with no proc-macro authoring, (2) doc comments on fields become `description` automatically, (3) `#[serde(default = …)]` is recognized." Acknowledge alternatives considered and rejected (custom proc-macro derive, runtime introspection only). The user's preference for full auto-derive is noted; schemars is the right call but the spec should say so deliberately.
- [ ] Add an AC for untagged-enum handling: "`schema_entries` for `WorkflowConfig` includes an entry for `workflow.states[].satisfies_deps` with a union-style `type_name` (e.g., `bool | string`), and `enum_variants = None`." The algorithm prose mentions this but it is not in the AC list, so an implementer could miss it.
