...
updated_at = "2026-04-09T06:30:42.613269Z"
+++
...
- [x] Entering anything other than "y" at the prompt leaves all epics untouched
- [x] `apm clean --epics` can be combined with other `apm clean` flags (e.g. `--branches`, `--dry-run`); epic cleanup runs after ticket cleanup in the same invocation
- [x] `apm clean --remote` (without `--epics`) also deletes "done" epic branches (both local and remote) as a final step after existing ticket cleanup
...
| 2026-04-09T06:18Z | in_progress | implemented | claude-0409-0610-6bc0 |
