...
updated_at = "2026-04-28T21:00:26.726564Z"
depends_on = ["50649e84"]
+++
...
- [x] `apm validate --fix` appends the worktree pattern and `# apm worktrees` comment to an existing `.gitignore` when they are absent
- [x] `apm validate --fix` creates `.gitignore` (with all standard APM entries including the worktree pattern) when the file is absent
...
**Ordering note:** this ticket depends on `50649e84` (merge `apm verify` into `apm validate`). The validate unit tests listed below live in `apm-core/src/validate.rs` (or its test module); once `50649e84` lands, `apm-core/tests/verify.rs` will be retired and the test surface lives entirely in `validate.rs`. Do not start implementation until `50649e84` is merged.
...
**Signature change** — `ensure_gitignore` becomes:

    pub fn ensure_gitignore(
        path: &Path,
        worktree_pattern: Option<&str>,
        messages: &mut Vec<String>,
    ) -> Result<()>

**Body change** — replace the static `entries` array with a `Vec<&str>`. Start with the 6 static entries, then conditionally push `"# apm worktrees"` and the owned pattern string when `worktree_pattern` is `Some`.
...
- `ensure_gitignore_worktrees_idempotent` — pass `Some("/worktrees/")` instead; update the assertion to check the pattern appears exactly once.
...
The existing `setup_gitignore_includes_worktrees_pattern` test needs no change (default config uses `dir = "worktrees"` which is in-repo, so `setup()` still writes `/worktrees/`).
...
Algorithm for `gitignore_covers_dir`: split file into lines; skip empty lines and comment lines (starting with `#`); trim whitespace from each remaining line; strip any leading and trailing `/`; compare the result to `dir` with its own leading/trailing `/` stripped. This ensures all four standard forms (`/<dir>/`, `/<dir>`, `<dir>/`, `<dir>`) match, while preventing a directory named `wt` from matching a line `wt-old` or vice-versa.
...
**New unit tests** — each uses a `TempDir`, creates an optional `.gitignore`, builds a minimal config TOML with `[worktrees] dir = "..."`, and calls `validate_config(&config, tmp.path())`:
...
- `validate_config_gitignore_no_false_positive` — `.gitignore` contains `wt-old/` and dir is `wt` -> error (prefix is not a match)
...
The `config` variable is in scope at this point in both `config_only` and full paths. The current invocation still exits with the error count after fixing (consistent with branch-field fix behaviour); a rerun of `validate` will pass.
...
- `WorktreesConfig::default()` in `config.rs` uses `"../worktrees"` (external), while `default_config()` in `init.rs` emits `dir = "worktrees"` (in-repo). The validate check fires for the in-repo case. Users who used `apm init` and never customised `worktrees.dir` get the check automatically.
- The idempotency check in `ensure_gitignore` is a `contents.contains(entry)` substring match (pre-existing). Do not change this in this ticket.
- `gitignore_covers_dir` normalises by stripping `/` from both sides of each line and from the configured dir, so `wt` cannot accidentally match `wt-old` (they are different after stripping).
- Implementation must not begin until ticket `50649e84` (verify -> validate merge) is merged; its landing moves the validate test surface that this ticket extends.
...
| 2026-04-28T20:21Z | in_design | specd | claude-0428-2017-76d0 |
| 2026-04-28T20:34Z | specd | ready | philippepascal |
| 2026-04-28T20:53Z | ready | in_progress | philippepascal |
| 2026-04-28T21:00Z | in_progress | implemented | claude-0428-2053-7030 |
