...
updated_at = "2026-05-04T19:41:29.930682Z"
epic = "5acea599"
...
- [x] `apm validate --verbose` is accepted without error on a valid project
- [x] Without `--verbose`, validate output is byte-for-byte identical to current behavior (no extra lines, no changed exit code)
- [x] The audit section lists exactly the transitions whose `trigger` equals `"command:start"`
- [x] For each spawn transition the text output shows: from-state ID, to-state ID, profile name (or none), resolved agent + source label, resolved instructions path or description + source label, resolved role prefix + source label, resolved wrapper
- [x] Source label for instructions is one of: `transition`, `profile:<name>`, `workers`, `project-agent-file`, `built-in`
- [x] Source label for role prefix is one of: `transition`, `profile:<name>`, `default`
- [x] Source label for agent is one of: `profile:<name>`, `workers`, `default`
- [x] When no `command:start` transitions exist, the audit section states "0 spawn transitions"
- [x] When a transition references a missing profile, the audit row shows "profile not found" for profile-dependent fields without panicking
- [x] `apm validate --verbose --json` adds an `"agent_resolution"` array to the JSON output; each element has `from_state`, `to_state`, `profile`, `agent`, `instructions`, `role_prefix`, `wrapper` keys; `agent`, `instructions`, and `role_prefix` each carry `value` and `source` subkeys
- [x] The code compiles and does not panic when `worker_profiles` is empty
...
| 2026-05-04T17:51Z | in_design | specd | claude-0504-1743-c4b0 |
