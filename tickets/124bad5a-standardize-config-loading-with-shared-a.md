...
updated_at = "2026-04-12T11:15:17.986708Z"
epic = "1e706443"
...
- [x] `util::blocking` accepts any `FnOnce() -> anyhow::Result<T>` that is `Send + 'static` and returns `Result<T, AppError>`
- [x] `util::blocking` does not require callers to double-`?` the result (`.await?` is sufficient)
- [x] `util::load_config` accepts a `PathBuf`, calls `Config::load` via `blocking`, and returns `Result<apm_core::config::Config, AppError>`
- [x] All async handlers in `main.rs` that previously called `Config::load` via inline `spawn_blocking` now use `util::load_config`
- [x] All async handlers in `agents.rs` (`get_agents_config`, `patch_agents_config`) use `util::load_config`
- [x] All async handlers in `work.rs` (`post_work_start`, `get_work_dry_run`) use `util::load_config`
- [x] `queue.rs` `queue_handler` uses `util::blocking` for its outer blocking closure (direct `Config::load` inside the closure is acceptable since it is already in a sync context)
- [x] `workers.rs` `workers_handler` uses `util::blocking` instead of bare `spawn_blocking`
- [x] No remaining `.await??` patterns appear in async handlers (all have been replaced by `.await?`)
- [x] Synchronous helper functions (`compute_blocking_deps`, `compute_valid_transitions`, `collect_workers`) are unchanged — they call `Config::load` directly as they run in a sync context
- [x] Startup initialization code (`build_app`, `setup_cors`) and test code are unchanged
...
| 2026-04-12T09:45Z | in_design | specd | claude-0412-0940-3ec8 |
