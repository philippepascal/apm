/// Asserts that `apm-core/src/default/agents/claude/apm.coder.md` and
/// `.apm/agents/claude/apm.coder.md` are byte-for-byte identical. Any
...
fn default_and_project_apm_worker_md_are_identical() {
    ...
    let default_path = std::path::Path::new(manifest_dir).join("src/default/agents/claude/apm.coder.md");
    let project_path = std::path::Path::new(manifest_dir)
        .parent()
        .expect("apm-core has a parent directory")
        .join(".apm/agents/claude/apm.coder.md");
    ...
    panic!(
        "apm-core/src/default/agents/claude/apm.coder.md and \
         .apm/agents/claude/apm.coder.md have diverged.\n\
...
fn default_and_project_apm_spec_writer_md_are_identical() {
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let default_path =
        std::path::Path::new(manifest_dir).join("src/default/agents/claude/apm.spec-writer.md");
    let project_path = std::path::Path::new(manifest_dir)
        .parent()
        .expect("apm-core has a parent directory")
...
/// Asserts that `apm-core/src/default/agents/claude/apm.main-agent.md` and
/// `.apm/agents/claude/apm.main-agent.md` are byte-for-byte identical. Any
/// divergence fails with a diff so the developer can see exactly what changed.
#[test]
fn default_and_project_apm_main_agent_md_are_identical() {
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let default_path =
        std::path::Path::new(manifest_dir).join("src/default/agents/claude/apm.main-agent.md");
    let project_path = std::path::Path::new(manifest_dir)
        .parent()
        .expect("apm-core has a parent directory")
        .join(".apm/agents/claude/apm.main-agent.md");
    ...
    panic!(
        "apm-core/src/default/agents/claude/apm.main-agent.md and \
         .apm/agents/claude/apm.main-agent.md have diverged.\n\
         Edit both files together to keep them in sync.\n\
         Diff (first differences):\n{}",
        diff_lines.join("\n")
    );
