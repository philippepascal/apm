...
pub fn setup(root: &Path, name: Option<&str>, description: Option<&str>, username: Option<&str>) -> Result<SetupOutput> {
    ...
    migrate_project_md_location(root, &apm_dir, &mut messages)?;
    migrate_agents_default_to_claude(root, &apm_dir, &mut messages)?;
    ...
    write_default(
        &agents_claude_dir.join("apm.main-agent.md"),
        include_str!("default/agents/claude/apm.main-agent.md"),
...
fn migrate_flat_agent_files(root: &Path, apm_dir: &Path, messages: &mut Vec<String>) -> Result<()> {
    ...
    let path_rewrites: &[(&str, &str)] = &[
        ...
        ("@.apm/agents/default/agents.md", "@.apm/project.md\n@.apm/agents/default/apm.main-agent.md"),
    ];
    ...
    let instructions_rewrites: &[(&str, &str)] = &[
        ...
        (".apm/style.md", ".apm/agents/default/style.md"),
        ("instructions = \".apm/agents/default/agents.md\"", "project = \".apm/project.md\""),
    ];
...
fn migrate_project_md_location(root: &Path, apm_dir: &Path, messages: &mut Vec<String>) -> Result<()> {
    let old_path = apm_dir.join("agents/default/apm.project.md");
    let new_path = apm_dir.join("project.md");
    if old_path.exists() && !new_path.exists() {
        std::fs::rename(&old_path, &new_path)?;
        messages.push("Moved .apm/agents/default/apm.project.md → .apm/project.md".to_string());
    }
    let claude_path = root.join("CLAUDE.md");
    if claude_path.exists() {
        let contents = std::fs::read_to_string(&claude_path)?;
        let updated = contents.replace("@.apm/agents/default/apm.project.md", "@.apm/project.md");
        if updated != contents {
            std::fs::write(&claude_path, &updated)?;
            messages.push("Updated CLAUDE.md (project → .apm/project.md)".to_string());
        }
    }
    let config_path = apm_dir.join("config.toml");
    if config_path.exists() {
        ...
        if updated != contents {
            std::fs::write(&config_path, &updated)?;
            messages.push("Updated config.toml (project → .apm/project.md)".to_string());
        }
    }
    Ok(())
}

/// Rename `.apm/agents/default/` → `.apm/agents/claude/` and update
/// CLAUDE.md and config.toml references.
fn migrate_agents_default_to_claude(root: &Path, apm_dir: &Path, messages: &mut Vec<String>) -> Result<()> {
    let default_dir = apm_dir.join("agents/default");
    let claude_dir = apm_dir.join("agents/claude");

    if !default_dir.exists() || claude_dir.exists() {
        return Ok(());
    }
    ...
    for file in &files {
        ...
        if old.exists() && !new.exists() {
            ...
            messages.push(format!("Moved .apm/agents/default/{file} → .apm/agents/claude/{file}"));
        }
    }
    ...
    if claude_md.exists() {
        let contents = std::fs::read_to_string(&claude_md)?;
        let mut updated = contents.clone();
        ...
        if updated != contents {
            ...
            messages.push("Updated CLAUDE.md (agents/default/ → agents/claude/)".to_string());
        }
    }
...
mod tests {
    ...
    fn setup_migrates_flat_agent_files_to_agents_default() {
        ...
        assert!(claude.contains("@.apm/agents/claude/apm.main-agent.md"));
        assert!(!claude.contains("@.apm/agents/claude/agents.md"));
        assert!(claude.contains("@.apm/agents/claude/style.md"));
        assert!(!claude.contains("@.apm/agents.md"));
    ...
    #[test]
    fn migrate_project_md_from_agents_default() {
        let tmp = TempDir::new().unwrap();
        git_init(tmp.path());
        std::fs::create_dir_all(tmp.path().join(".apm/agents/default")).unwrap();
        std::fs::write(tmp.path().join(".apm/agents/default/apm.project.md"), "old location").unwrap();
        std::fs::write(tmp.path().join("CLAUDE.md"), "@.apm/agents/default/apm.project.md\n").unwrap();
        std::fs::write(
            tmp.path().join(".apm/config.toml"),
            "[project]\nname = \"test\"\ndefault_branch = \"main\"\n\n[agents]\nproject = \".apm/agents/default/apm.project.md\"\n",
        ).unwrap();
